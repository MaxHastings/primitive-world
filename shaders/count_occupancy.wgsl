...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  let wrapped = wrap_world(agents[index].position, params.world_size.xy);
  let cell = clamp(floor(wrapped / params.world_size.xy * f32(GRID)), vec2<f32>(0.0), vec2<f32>(f32(GRID - 1u)));
