...
fn main(@builtin(global_invocation_id) id:vec3<u32>){
 ...
 if(a.alive==0u){destination[i]=a;return;}let d=decisions[i];a.lived_ticks++;
 // This pass is dispatched over compact living indices. Lane zero runs once
 // whenever any body was alive at tick start; empty trailing ticks do not score.
 ...
 // Gathering is chosen; every absorbed unit is removed from inventory.
 {
  ...
  let food_units=u32(round(amount*1000.0));
  let food_before=atomicAdd(&stats[0],food_units);
  if(food_before>0xffffffffu-food_units){atomicAdd(&stats[14],1u);}
 }
