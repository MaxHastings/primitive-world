...
fn main(@builtin(global_invocation_id) id:vec3<u32>){
 ...
 x[14]=f32(a.action)/6.0;x[15]=a.attention/3.14159265;
 for(var k=0u;k<8u;k++){x[16u+k*3u]=p.samples[k].food;x[17u+k*3u]=p.samples[k].offset.x/a.sensor_radius;x[18u+k*3u]=p.samples[k].offset.y/a.sensor_radius;}
 for(var k=0u;k<4u;k++){let b=p.bodies[k];let n=40u+k*6u;
 ...
 for(var k=0u;k<64u;k++){if(!finite(x[k])){d.invalid=1u;}x[k]=clamp(x[k],-8.0,8.0);d.inputs[k]=x[k];}
 for(var h=0u;h<16u;h++){
  let row=h*81u;var v=genomes[i*1568u+row+80u];
  for(var k=0u;k<64u;k++){v+=genomes[i*1568u+row+k]*x[k];}
  for(var k=0u;k<16u;k++){v+=genomes[i*1568u+row+64u+k]*a.hidden[k];}
  if(!finite(v)){d.invalid=1u;v=0.0;}d.hidden[h]=tanh(v);
 }
 var out:array<f32,16>;
 for(var o=0u;o<16u;o++){
  let row=1296u+o*17u;var v=genomes[i*1568u+row+16u];
  for(var h=0u;h<16u;h++){v+=genomes[i*1568u+row+h]*d.hidden[h];}
 ...
 if(d.invalid!=0u){d.selected_action=NONE;d.movement=vec2<f32>(0);d.amount=0.0;d.attention=0.0;d.payload=0.0;for(var h=0u;h<16u;h++){d.hidden[h]=0.0;}}
 decisions[i]=d;
}
