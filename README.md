...
The optional `--neural` flag enables a compact recurrent action policy while leaving the authored local-rule policy as the default. It has private per-agent memory and receives only local perception and social observations; action masks preserve the same physical accounting. The shipped weights are a neutral integration baseline, not trained competence. See [NEURAL_POLICY.md](NEURAL_POLICY.md) for export, offline training, and loading commands.
