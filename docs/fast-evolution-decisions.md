...
| 9 | Parallelize the existing 512-cell food sensing calculation across eight lanes per organism; keep dense spatial linking and telemetry. | Three short paired checkpoint replays raised median headless throughput from 1,391 to 1,652 macro steps/s. All channels matched the serial shader within 2.38e-7 on an evolved 736-organism/110-packet checkpoint, with no first-step action changes. Compact spatial linking regressed; sparse telemetry showed no consistent extra gain. See [the follow-up](sensing-fixed-work-20260923.md). | Parallel summation can change later floating-point trajectories. Speed is measured; strategy discovery per real hour still requires the continuing wallpaper run. |
