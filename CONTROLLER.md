...
Birth inheritance and [offline preparation](training/TRAINING_PLAN.md) are distinct:
the persistent trainer evaluates candidates' families, preserves their initial
genomes even through extinction, then retains/mutates candidates for new worlds.
It does not carry experiences or automatically pick a surviving child's genome.
No prepared primitive-v3 bank is silently bundled. Random does not mean competent.

The historical [authored starter](training/STARTER_PLAN.md) is no longer recommended.
It supplied collection, reserve-conditioned reproduction and local food steering.
Its lower social scores were not merely a preference: transfer, force and emission
were provably outscored for every possible hidden state in all original genomes
and all 139 survivors of the inspected user save. Mutation remained possible, but
within-life trials of these actions were impossible with those fixed weights.
Historical artifacts remain usable explicitly; defaults and future recommended
training starts use random weights. This does not add stochastic action selection
or within-life weight learning. See reports/RANDOM_ORIGIN_DECISION.md.
